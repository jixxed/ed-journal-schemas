...
try {
    ...
    function processSchema(schemaPath) {
        ...
        const processedSchema = {
            ...
            propertiesString: allProperties.map(prop => prop.name + (prop.properties) ? prop.properties.map(prop => prop.name + (prop.properties) ? prop.properties.map(prop => prop.name).join(' ') : '').join(' ') : '').join(' '),
            propertiesDescriptionString: allProperties.map(prop => prop.description + (prop.properties) ? prop.properties.map(prop => prop.description + (prop.properties) ? prop.properties.map(prop => prop.description).join(' ') : '').join(' ') : '').join(' '),
        };
